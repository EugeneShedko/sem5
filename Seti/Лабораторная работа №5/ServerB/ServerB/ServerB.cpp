...
int main()
{
	...
	try
	{
		...
		char hostname[50];
		int lb2 = 0;
		if ((lb2 = gethostname(hostname, sizeof(hostname))) == SOCKET_ERROR)
		{
			throw SetErrorMsgText("gethostname", WSAGetLastError());
		}
		cout << "Имя хоста: " << hostname << endl;
		const char *ip;
		hostent* compname;
		DWORD dwordip;
		while (true)
		{
			if (GetRequestFromClient("Hello", 2000, (sockaddr*)&clnt, &lc, sS))
			{
				...
				compname = gethostbyaddr((char*)&dwordip, 4 , AF_INET);
				cout << "Client Name: " << compname->h_name << endl;
			}
