...
int main()
{
	...
	try
	{
		...
		while (true)
		{
			...
			{
				throw SetErrorMsgText("recvfrom: ", GetLastError());
			}
			int lobuf = 0;
			if (strlen(ibuf) == 0)
			{
			...
			cout << "Сообщение: " << ibuf << endl;
			string str = "zzzz";
			if ((lobuf = sendto(sS, str.c_str(), sizeof(str), NULL, (sockaddr*)&clnt, sizeof(clnt))) == SOCKET_ERROR)
			{
