...
--1
create or replace procedure proc1 (name customers.company%type)
is
    cursor curs is select p.DESCRIPTION, c.company from PRODUCTS p inner join orders o on p.PRODUCT_ID=o.product
    inner join customers c on o.cust=c.cust_num
        where c.company=name;
    nameC customers.company%type;
    nameP PRODUCTS.DESCRIPTION%type;
    c int;
begin
    open curs;
    loop 
        fetch curs into nameP, nameC;
        exit when curs%notfound;
        dbms_output.put_line(nameP || ' ' || nameC);
    end loop;
    close curs;
    select avg(o.amount) into c from orders o inner join customers c on o.cust=c.cust_num
        where c.company=name;
    dbms_output.put_line(c);
end;

begin 
    proc1('Fred Lewis Corp.');
end;

--2
create or replace function func2 (name customers.company%type, d1 date, d2 date) return int
is
    c int;
begin
    select count(*) into c from orders o inner join customers c on o.cust=c.cust_num
        where c.company=name and order_date>'2000-12-17' and order_date<'2020-12-17';
    return c;
end;

begin 
    dbms_output.put_line(func2('Fred Lewis Corp.','2000-12-17','2020-12-17'));
end;
