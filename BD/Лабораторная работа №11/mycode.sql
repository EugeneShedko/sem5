...
create or replace procedure userorders(username in varchar(20))
is
  cursor average_cursor(namee C##SEA.customers.company%type) is select customers.company, orders.order_num, orders.order_date, orders.mfr, orders.product, orders.qty, orders.amount from customers inner join 
orders on customers.cust_num = orders.cust where customers.company = namee;
  summ number(10) := 0;
  countt number(10) := 0;
  average number(10) := 0;
begin
  select customers.company, orders.order_num, orders.order_date, orders.mfr, orders.product, orders.qty, orders.amount from customers inner join 
  orders on customers.cust_num = orders.cust where customers.company = username;
  
  for k in average_cursor(username)
  loop
    summ := summ + k.amount;
    countt := countt + 1;
    dbms_output.put_line(k.amount);
  end loop;
  average := summ / countt;
...
orders on customers.cust_num = orders.cust where customers.company = namee;
  ...
  average number(10) := 0;
begin
  for k in average_cursor('JCP Inc.')
  loop
    summ := summ + k.amount;
    countt := countt + 1;
    dbms_output.put_line(k.amount);
...
select customers.company, orders.order_num, orders.order_date, orders.mfr, orders.product, orders.qty, orders.amount, avg(orders.amount) as sss from customers inner join 
orders on customers.cust_num = orders.cust where customers.company = 'JCP Inc.';
